...
class TestChecker(
  ...
  UtestFramework.recordedSuiteDescriptions(suiteName.stripSuffix("Tests$")) =
    description

  val autoCommitConnection = dbClient.getAutoCommitClientConnection
  def reset() = {
    autoCommitConnection.updateRaw(
      os.read(os.pwd / "scalasql-tests/src/test/resources" / testSchemaFileName)
    )

    autoCommitConnection.updateRaw(
      os.read(os.pwd / "scalasql-tests/src/test/resources" / testDataFileName)
    )

  }

  def recorded[T](docs: String, f: sourcecode.Text[T])(implicit
      tp: utest.framework.TestPath
  ): T = {
    val res = f.value
  ...
  def apply[T, V](
    ...
    if (allCheckedSqls.nonEmpty) {
      assert(matchedSql.nonEmpty, pprint.apply(sqlResult))
    }
    ...
    if (values.nonEmpty) {
      assert(
        values.exists(value => normalized == value),
        pprint.apply(normalized)
      )
    }
